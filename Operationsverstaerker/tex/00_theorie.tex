...
\subsection{Ideale und reale Operationsverstärker}
...
Für reale Operationsverstärker gibt es weiete Eigenschaften, die erläutert werden müssen:
...
Außerdem lässt socj dem Offsetstrom definieren

\begin{equation}
I_0 = I_P - I_N
\end{equation}

Aus den definierten Größen lässt socj der Differenzeingangswiderstand $r_D$ definieren:

\begin{equation}
r_D = \left\{\begin{array}{ll} \frac{\delat U_P}{\delta I_P}, für U_N = 0\\
	  \frac{\delta U_N}{\delta I_N}, für U_P = 0\end{array}\right.
\end{equation}

Auch ist für $U_G = U_P = U_N$ und $I_G = I_P + I_N$ der Gleichtackteingangswiderstand
...
\begin{equation}
\frac{1}{V^'} = - \frac{U_1}{U_A} \approx \frac{1}{V} + \frac{R_1}{R_N}
\end{equation}
...
\begin{equation}
U_A = - \frac{1}{RC} \int_{0}^{\frac{T}{2}} U_E(t^')dt^' = -\frac{1}{RC} U_B \frac{T}{2}
\end{equation} 
